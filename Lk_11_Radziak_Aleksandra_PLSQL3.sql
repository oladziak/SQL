...
INSERT INTO LEKARZ (LEK_Imie, LEK_Nazwisko)
	VALUES ('Piotr', 'Nowak');
INSERT INTO LEKARZ (LEK_Imie, LEK_Nazwisko, LEK_Specjalnosc)
...
PROMPT----------------------------------------;
PROMPT ;
COLUMN PAC_Imie1 Heading 'Imie1' format A15
COLUMN PAC_Imie2 Heading 'Imie2' format A15
COLUMN PAC_Nazwisko Heading 'Nazwisko' format A15
COLUMN ADR_Miejscowosc Heading 'Miejscowosc' format A18
COLUMN ADR_Ulica Heading 'Ulica' format A18
COLUMN ADR_Nr_domu Heading 'Nr_domu' format 9999999
...
(Imie1, Imie2, Nazwisko, Miejscowosc, Ulica, Nr_domu)
as
select p.PAC_Imie1, p.PAC_Imie2, p.PAC_Nazwisko, a.ADR_Miejscowosc, a.ADR_Ulica, a.ADR_Nr_domu
FROM PACJENT p, ADRES a
WHERE p.ADR_Id = a.ADRk_1_Id AND p.PAC_Nazwisko = (SELECT PAC_Nazwisko FROM PACJENT WHERE PAC_Nazwisko LIKE 'N%');

COLUMN LEK_Imie Heading 'Imie' format A15
COLUMN LEK_Nazwisko Heading 'Nazwisko' format A15
COLUMN LEK_Specjalnosc Heading 'Specjalnosc' format A25
COLUMN ODD_Nazwa Heading 'Nazwa_oddzialu' format A28

create or replace view V_LEK_ODD
(Imie, Nazwisko, Specjalnosc, Nazwa_oddzialu)
as
select l.LEK_Imie, l.LEK_Nazwisko, l.LEK_Specjalnosc, od.ODD_Nazwa
FROM LEKARZ l, ODDZIAL od
WHERE l.LEKk_1_Id = od.LEK_Id AND od.ODD_Nazwa = (SELECT ODD_Nazwa FROM ODDZIAL WHERE ODD_Nazwa LIKE 'R%');


select * from V_LEK_ODD;
--
PROMPT ;
PROMPT----------------------------------------;
PROMPT PROCEDURE update_pac;
PROMPT----------------------------------------;
PROMPT ;
--
...
CREATE OR REPLACE PROCEDURE UPDATE_PAC(inNazwisko IN PACJENT.PAC_Nazwisko%TYPE, inImie2 IN PACJENT.PAC_Imie2%TYPE)
IS
	PRAGMA AUTONOMOUS_TRANSACTION;
	CURSOR curPAC_IMIE2(cNazwisko IN PACJENT.PAC_Nazwisko%TYPE)
	IS
	SELECT * FROM PACJENT 
		WHERE PAC_Nazwisko = cNazwisko
		FOR UPDATE of PAC_Imie2;
	
	Pwiersz PACJENT%ROWTYPE;
	
	ile number := 0;
	status number :=0;
BEGIN
	OPEN curPAC_IMIE2(inNazwisko);
	
	LOOP
	FETCH curPAC_IMIE2 INTO Pwiersz;
		--
		EXIT WHEN curPAC_IMIE2%NOTFOUND OR curPAC_IMIE2%ROWCOUNT <1;
		--
		ile:=ile+1;
			IF Pwiersz.PAC_Imie2 IS NULL THEN
				IF inImie2 IS NOT NULL THEN
					--kursor niejawny
					UPDATE PACJENT SET PAC_Imie2 = inImie2
					WHERE CURRENT OF curPAC_IMIE2;
	...
	IF ile=status THEN
		DBMS_OUTPUT.PUT_LINE('UPDATE_PAC: COMMIT');
		COMMIT;
	ELSE
		DBMS_OUTPUT.PUT_LINE('UPDATE_PAC: ROLLBACK');
		ROLLBACK;
	END IF;
END;
/
...
BEGIN
UPDATE_PAC('Nowak', 'Malgorzata');
end;
/			

SELECT * FROM V_PAC_ADR;
	
--
PROMPT ;
PROMPT----------------------------------------;
PROMPT PROCEDURE update_lek;
PROMPT----------------------------------------;
PROMPT ;
--

CREATE OR REPLACE PROCEDURE UPDATE_LEK(inNazwisko IN LEKARZ.LEK_Nazwisko%TYPE, inSpecjalnosc IN LEKARZ.LEK_Specjalnosc%TYPE)
IS
	PRAGMA AUTONOMOUS_TRANSACTION;
	CURSOR curLEK_Specjalnosc(cNazwisko IN LEKARZ.LEK_Nazwisko%TYPE)
	IS
	SELECT * FROM LEKARZ 
		WHERE LEK_Nazwisko=cNazwisko
		FOR UPDATE OF LEK_Specjalnosc;
	
	Lwiersz LEKARZ%ROWTYPE;
	ile number := 0;
	status number :=0;
BEGIN
	OPEN curLEK_Specjalnosc(inNazwisko);
	
	LOOP
	FETCH curLEK_Specjalnosc INTO Lwiersz;
	--
		EXIT WHEN curLEK_Specjalnosc%NOTFOUND OR curLEK_Specjalnosc%ROWCOUNT <1;
		--
		ile:=ile+1;
			IF Lwiersz.LEK_Specjalnosc IS NULL THEN
				IF inSpecjalnosc IS NOT NULL THEN
					--kursor niejawny
					UPDATE LEKARZ SET LEK_Specjalnosc = inSpecjalnosc
					WHERE CURRENT OF curLEK_Specjalnosc;
	...
	END LOOP;
	--
	CLOSE curLEK_Specjalnosc;
	
	IF ile=status THEN
		DBMS_OUTPUT.PUT_LINE('UPDATE_LEK: COMMIT');
		COMMIT;
	ELSE
		DBMS_OUTPUT.PUT_LINE('UPDATE_LEK: ROLLBACK');
		ROLLBACK;
	END IF;
END;
/
