...
COLUMN ADR_Nr_domu Heading 'Nr domu' format 9999999
...
--
COLUMN GRUk_1_Id Heading 'Id sali' format 9999999
COLUMN SAL_Nr_sali Heading 'Nr sali' format 9999999
COLUMN PIE_Imie Heading 'Imie pielegniarki' format A18
COLUMN PIE_Nazwisko Heading 'Nazwisko' format A18

SELECT g.GRUk_1_Id, 
s.SAL_Nr_sali, 
p.PIE_Imie, 
p.PIE_Nazwisko
FROM GRUPA g, SALA s, PIELEGNIARKA p
WHERE s.SALk_1_Id= g.SAL_Id AND g.PIE_Id= p.PIEk_1_Id AND PIE_Imie LIKE 'A%';

PROMPT ;
PROMPT----------------------------------------;
PROMPT Select with GROUP BY and HAVING;
PROMPT----------------------------------------;
PROMPT ;

COLUMN PAC_Id Heading 'Id' format 999

SELECT PAC_Id,
COUNT(PAC_Id) ilosc_przyjec
FROM PRZYJECIA 
HAVING MIN(PAC_Id)>2
GROUP BY PAC_Id;
--
COLUMN LEK_Specjalnosc Heading 'Specjalnosc' format A15

SELECT LEK_Specjalnosc,
COUNT(LEK_Specjalnosc) ilosc_lekarzy
FROM LEKARZ 
HAVING LEK_Specjalnosc LIKE 'k%'
GROUP BY LEK_Specjalnosc;
--
COLUMN BAD_Nazwa Heading 'Nazwa badania' format A30

SELECT BAD_Nazwa,
AVG(BAD_Wynik) wynik
FROM BADANIE 
HAVING MAX(BAD_Wynik)<150
GROUP BY BAD_Nazwa;

PROMPT ;
PROMPT----------------------------------------;
PROMPT OPERATORY ZBIOROWE;
PROMPT----------------------------------------;
PROMPT ;
COLUMN PACk_1_Id Heading 'ID' format 99
COLUMN PAC_Imie1 Heading 'Imie 1' format A18
COLUMN PAC_Imie2 Heading 'Imie 2' format A18
COLUMN PAC_Pesel Heading 'PESEL' format 999999999999

SELECT PACk_1_Id, PAC_Imie1, PAC_Pesel
FROM PACJENT WHERE PACk_1_Id BETWEEN 1 AND 3
MINUS 
SELECT PACk_1_Id, PAC_Imie1, PAC_Pesel
FROM PACJENT WHERE PACk_1_Id BETWEEN 2 AND 4
ORDER BY 1;

SELECT PACk_1_Id, PAC_Imie1, PAC_Pesel
FROM PACJENT WHERE PACk_1_Id BETWEEN 1 AND 3
UNION 
SELECT PACk_1_Id, PAC_Imie2, PAC_Pesel
FROM PACJENT WHERE PACk_1_Id BETWEEN 2 AND 4;
--
...
COLUMN ADR_Nr_lokalu Heading 'Nr lokalu' format 9999999999

SELECT ADR_Miejscowosc, ADR_Ulica, ADR_Nr_lokalu
FROM ADRES WHERE ADR_Miejscowosc LIKE 'W%'
UNION
SELECT ADR_Miejscowosc, ADR_Ulica, ADR_Nr_domu
FROM ADRES WHERE ADR_Miejscowosc LIKE 'N%' ;
