...
WHERE p.ADR_Id = a.ADRk_1_Id AND a.ADR_Ulica = (SELECT ADR_Ulica FROM ADRES WHERE ADR_Ulica LIKE 'Rej%');
...
CREATE or REPLACE PROCEDURE INSERT1_ADR_PAC(
...
BEGIN
	...
	--
	SELECT SEQ_PACJENT.CURRVAL INTO PAC_ID_curr FROM DUAL;
	--
	DBMS_OUTPUT.PUT_LINE('Dodano nowy wiersz do tableki ADRES z ID='||ADR_ID_curr);
	--
	DBMS_OUTPUT.PUT_LINE('Dodano nowy wiersz do tabeli PACJENT z ID='||PAC_ID_curr);
	--
	IF ADR_ID_curr>0 AND PAC_ID_curr>0 THEN	
		COMMIT;
	ELSE
		ROLLBACK;
	END IF;
...
BEGIN
	INSERT1_ADR_PAC('Polska', 'Katowice', 'Rejtana', 28, '32-065', 'Bernardeta',
	'Kulig', 89022204560, '1989-02-22', 'Katowice');
END;
/
	--check:
	SELECT * FROM V_PAC_ADR;

CREATE or REPLACE PROCEDURE INSERT2_ADR_PAC(
inADR_KRAJ in ADRES.ADR_Kraj%TYPE,
inADR_MIEJ in ADRES.ADR_Miejscowosc%TYPE,
inADR_ULICA in ADRES.ADR_Ulica%TYPE,
inADR_NRDOMU in ADRES.ADR_Nr_domu%TYPE,
...
BEGIN
	...
	IF SQL%FOUND THEN
		...
		IF SQL%FOUND THEN 
			...
			COMMIT;
		ELSE
			DBMS_OUTPUT.PUT_LINE('Procedura insert2_adr_pac daje ERROR!');
			ROLLBACK;
		END IF;
	ELSE
		ROLLBACK;
...
BEGIN
	INSERT2_ADR_PAC('Polska', 'Lublin', 'Lwowska', 9, '31-234', 
	'Bartosz', 'Karpiel', 82031804677, '1982-03-18', 'Lublin');
END;
/
